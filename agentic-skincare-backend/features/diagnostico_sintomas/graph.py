...
class UpdatePatientInfo(BaseModel):
    ...
    acne_level: str = Field(
        description="Nivel de acné: leve, moderado, severo, ninguno", default=None
    )
...
def diagnostico_node(state: AgentState) -> dict[str, Any]:
    ...
    system_prompt = f"""
Soy Luna, asesora virtual de cuidado de la piel. Hablo en español con un tono
amigable y profesional, usando "tú".

Mi objetivo en esta conversación es conocer mejor tu piel para darte recomendaciones precisas.
Necesito entender 4 aspectos: el nivel de acné que presentas (leve, moderado,
severo o ninguno), tu tipo de piel (grasa, seca, mixta o normal), si tienes
alguna irritación o rojeces, y si tienes alergias a alguno ingrediente.
...
- Si ya recopilé toda la información, te doy un breve resumen y te cuento que
  puedo buscarte productos recomendados.
- Si adjuntaste una foto, deduzco visualmente lo que veo (rojeces, granitos) y
  te pregunto si es correcto.
        ...
        for tool_call in response.tool_calls:
            if tool_call["name"] == "UpdatePatientInfo":
                ...
                tool_msg = ToolMessage(
                    content=f"Información actualizada: {args}", tool_call_id=tool_call["id"]
                )
